// app/awc-test-preparation/assistant-manager-software/page.tsx
'use client'
...
export default function SoftwareTestPage() {
  ...
  const loadMCQs = async (selectedDifficulty: 'easy' | 'medium' | 'hard') => {
    ...
    setError(null)
    try {
      const data = await fetchMCQs('physics', selectedDifficulty, 60)
      if (data.length < 60) {
  ...
  if (testStarted && mcqs.length > 0) {
    return <QuizInterface mcqs={mcqs} category="physics" difficulty={difficulty!} />
  }

  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-purple-50 dark:from-gray-900 dark:via-black dark:to-gray-900 py-12 px-4">
      <div className="max-w-5xl mx-auto">
        {/* Back Button */}
        <Link
          href="/awc-test-preparation"
          className="inline-flex items-center gap-2 text-purple-600 dark:text-purple-400 hover:text-purple-700 dark:hover:text-purple-300 mb-8 font-semibold"
        >
        ...
        <div className="text-center mb-12">
          <div className="inline-block p-6 bg-gradient-to-r from-purple-500 to-purple-600 rounded-2xl shadow-2xl mb-6">
            <Atom className="w-16 h-16 text-white" />
          </div>
          <h1 className="text-4xl md:text-5xl font-black text-gray-900 dark:text-white mb-4">
            Assistant Manager Physics Test
          </h1>
